...
const PREDEFINED_ENTRIES = [{ empresa: 'FIBER' }, { empresa: 'CAMARGOS' }, { empresa: 'WORLD WIDE' }];
const FIXED_BANKS = ['Inter', 'Itaú', 'Caixa'];

const formatCurrency = (value: number) => value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

export const PrevisaoCristiano: React.FC = () => {
    const [previsoes, setPrevisoes] = useState<Previsao[]>(() => {
        const saved = localStorage.getItem('previsoes_cristiano');
    ...
    const [closedDates, setClosedDates] = useState<Set<string>>(() => new Set(JSON.parse(localStorage.getItem('closedDates_cristiano') || '[]')));

    useEffect(() => { localStorage.setItem('previsoes_cristiano', JSON.stringify(previsoes)); }, [previsoes]);
    useEffect(() => { localStorage.setItem('closedDates_cristiano', JSON.stringify(Array.from(closedDates))); }, [closedDates]);
    ...
    const handleAddNewEntry = () => {
        ...
        setPrevisoes(prev => [...prev, { ...newEntry, id: `prev-cris-${Date.now()}` } as Previsao]);
    ...
    return (
        <div className="p-4 sm:p-6 w-full h-full overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-text-primary">Previsão Cristiano</h3>
