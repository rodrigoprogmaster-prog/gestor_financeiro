...
// Data structure
interface WorldWideTransaction {
  id: string;
...
const STORAGE_KEY_WW = 'cartao_worldwide_data';

const createTransactionKey = (transaction: Omit<WorldWideTransaction, 'id' | 'Status'>): string => {
...
const parseCsvData = (csvText: string, headersToKeep: string[]): Omit<WorldWideTransaction, 'id' | 'Status'>[] => {
    ...
    const data = rows.slice(1).map(row => {
        ...
        return transaction as Omit<WorldWideTransaction, 'id' | 'Status'>;
    });

    return data;
};

interface CartaoWorldWideProps {
  onBack: () => void;
}

const CartaoWorldWide: React.FC<CartaoWorldWideProps> = ({ onBack }) => {
    const [data, setData] = useState<WorldWideTransaction[]>(() => {
        const saved = localStorage.getItem(STORAGE_KEY_WW);
    ...
    useEffect(() => {
        localStorage.setItem(STORAGE_KEY_WW, JSON.stringify(data));
    }, [data]);
    ...
    const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const headersToKeep = ['Data da Transação', 'Transação', 'Valor original'];
                ...
                const newData: WorldWideTransaction[] = dataWithDefaultCategory.map((item) => {
    ...
    const handleExportXLSX = () => {
        ...
        XLSX.utils.sheet_add_aoa(worksheet, [[null, 'Total Lançamentos:', totals.transactionCount, 'Valor Total:', totals.totalValue]], { origin: -1 });
        
        const range = XLSX.utils.decode_range(worksheet['!ref']);
        for (let R = 1; R < range.e.r; ++R) {
            const cell_ref = XLSX.utils.encode_cell({c: 2, r: R}); // Column C
            if(worksheet[cell_ref]) {
                worksheet[cell_ref].t = 'n';
                worksheet[cell_ref].z = 'R$ #,##0.00';
            }
        }
        const totalCellRef = XLSX.utils.encode_cell({c: 4, r: range.e.r});
        if(worksheet[totalCellRef]) {
            worksheet[totalCellRef].t = 'n';
            worksheet[totalCellRef].z = 'R$ #,##0.00';
        }

        worksheet['!cols'] = [{ wch: 20 }, { wch: 40 }, { wch: 15 }, { wch: 30 }, { wch: 15 }];

        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Fatura World Wide');
        XLSX.writeFile(workbook, `fatura_worldwide_${monthFilter || 'geral'}_${new Date().toISOString().slice(0,10)}.xlsx`);
    };
        ...
        <div className="animate-fade-in flex flex-col h-full">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-xl md:text-2xl font-bold text-text-primary">Cartão World Wide</h3>
                </div>
                            ...
                            {filteredData.length > 0 ? (
                                filteredData.map(item => (
                                    <tr key={item.id} onClick={() => handleStatusToggle(item.id)} className="bg-card border-b border-border hover:bg-secondary cursor-pointer transition-colors duration-200">
                                        ...
                                        <td className="px-6 py-2.5 whitespace-nowrap text-text-primary font-medium">{item['Transação']}</td>
...
export default CartaoWorldWide;
