...
export const PrevisaoFabrica: React.FC = () => {
    const LS_KEY_PREVISOES = 'previsoes_fabrica';
    const LS_KEY_CLOSED_DATES = 'closedDates_fabrica';
    const LS_KEY_HISTORICO_GERADAS = 'historicoPrevisoesGeradas_fabrica';
    const LS_KEY_VIEW = 'previsaoFabrica_view';
    const LS_KEY_DATE_FILTER = 'previsaoFabrica_dateFilter';
    const LS_KEY_REPORT_DATE_FILTER = 'previsaoFabrica_reportDateFilter';
    const LS_KEY_REPORT_WEEK_FILTER = 'previsaoFabrica_reportWeekFilter';
    const LS_KEY_DASHBOARD_SEMANA_FILTER = 'previsaoFabrica_dashboardSemanaFilter';
    const LS_KEY_PREVISAO_GERADA_ATIVA = 'previsaoFabrica_previsaoGeradaAtiva';


    const [previsoes, setPrevisoes] = useState<Previsao[]>(() => {
        const savedPrevisoes = localStorage.getItem(LS_KEY_PREVISOES);
    ...
    const [dateFilter, setDateFilter] = useState<string>(() => localStorage.getItem(LS_KEY_DATE_FILTER) || new Date().toISOString().split('T')[0]);
    const [closedDates, setClosedDates] = useState<Set<string>>(() => {
        const savedClosedDates = localStorage.getItem(LS_KEY_CLOSED_DATES);
        return savedClosedDates ? new Set(JSON.parse(savedClosedDates)) : new Set();
    });
    
    const [view, setView] = useState<View>(() => (localStorage.getItem(LS_KEY_VIEW) as View) || 'menu');
    
    const [reportDateFilter, setReportDateFilter] = useState<string>(() => localStorage.getItem(LS_KEY_REPORT_DATE_FILTER) || '');
    const [reportWeekFilter, setReportWeekFilter] = useState<string>(() => localStorage.getItem(LS_KEY_REPORT_WEEK_FILTER) || '');
    ...
    const [previsaoGeradaAtiva, setPrevisaoGeradaAtiva] = useState<PrevisaoGerada | null>(() => {
        try {
            const saved = localStorage.getItem(LS_KEY_PREVISAO_GERADA_ATIVA);
            return saved ? JSON.parse(saved) : null;
        } catch (e) {
            console.error("Failed to parse previsaoGeradaAtiva from localStorage", e);
            return null;
        }
    });
    const [historicoPrevisoesGeradas, setHistoricoPrevisoesGeradas] = useState<PrevisaoGerada[]>([]);
    const [dashboardSemanaFilter, setDashboardSemanaFilter] = useState(() => localStorage.getItem(LS_KEY_DASHBOARD_SEMANA_FILTER) || '');
    ...
    useEffect(() => {
        localStorage.setItem(LS_KEY_PREVISOES, JSON.stringify(previsoes));
    }, [previsoes]);

    useEffect(() => {
        localStorage.setItem(LS_KEY_CLOSED_DATES, JSON.stringify(Array.from(closedDates)));
    }, [closedDates]);

    useEffect(() => {
      const savedGeneratedHistory = localStorage.getItem(LS_KEY_HISTORICO_GERADAS);
    ...
    useEffect(() => {
        if (historicoPrevisoesGeradas.length > 0 || localStorage.getItem(LS_KEY_HISTORICO_GERADAS)) {
            localStorage.setItem(LS_KEY_HISTORICO_GERADAS, JSON.stringify(historicoPrevisoesGeradas));
        }
    }, [historicoPrevisoesGeradas]);
    
    useEffect(() => {
        localStorage.setItem(LS_KEY_VIEW, view);
    }, [view]);

    useEffect(() => {
        localStorage.setItem(LS_KEY_DATE_FILTER, dateFilter);
    }, [dateFilter]);

    useEffect(() => {
        localStorage.setItem(LS_KEY_REPORT_DATE_FILTER, reportDateFilter);
    }, [reportDateFilter]);

    useEffect(() => {
        localStorage.setItem(LS_KEY_REPORT_WEEK_FILTER, reportWeekFilter);
    }, [reportWeekFilter]);

    useEffect(() => {
        localStorage.setItem(LS_KEY_DASHBOARD_SEMANA_FILTER, dashboardSemanaFilter);
    }, [dashboardSemanaFilter]);

    useEffect(() => {
        localStorage.setItem(LS_KEY_PREVISAO_GERADA_ATIVA, JSON.stringify(previsaoGeradaAtiva));
    }, [previsaoGeradaAtiva]);
