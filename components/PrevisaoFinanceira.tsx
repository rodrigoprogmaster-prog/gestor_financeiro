...
const PrevisaoFabrica: React.FC = () => {
    ...
    useEffect(() => {
      const savedHistory = localStorage.getItem('historicoPrevisoes');
...
// @FIX: Provide a typed initial value to the reduce function to prevent type inference issues.
    ...
    const handleAddDay = () => {
        ...
        const action = () => {
            const newEntries: Previsao[] = PREDEFINED_ENTRIES.map((entry, index) => ({
                id: Date.now() + index,
                data: newDayDate,
                semana: newDaySemana,
    ...
    const handleFecharDia = () => {
        ...
        const action = () => {
            const existingHistory: Previsao[] = JSON.parse(localStorage.getItem('historicoPrevisoes') || '[]');
            const updatedHistory = [...existingHistory, ...filteredPrevisoes];
            localStorage.setItem('historicoPrevisoes', JSON.stringify(updatedHistory));
